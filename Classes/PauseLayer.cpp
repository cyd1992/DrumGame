...
PauseLayer* PauseLayer::createPause()
{
	PauseLayer * ret = new (std::nothrow) PauseLayer();
	if (ret && ret->initPause())
	{
		ret->autorelease();
	}
	else
...
bool PauseLayer::initPause()
{
	...
	auto resumeItem = MenuItemImage::create("guide/restart.png", "main/restart_sel.png", [this](Ref* sender) {
		PracticeGameScene* scene = (PracticeGameScene*)(this->getParent());
	...
	auto backItem = MenuItemImage::create("guide/return.png", "guide/return_sel.png", [this](Ref* sender) {
		PracticeGameScene* scene = (PracticeGameScene*)(this->getParent());
